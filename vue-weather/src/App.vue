<template>
  <div id="app" :class="typeof weather.main != 'undefined' && weather.main.temp > 16 ? 'warm' : 'cold'">
    <main>
      <div class="search-box">
        <input type="text" v-model="area" class="search-bar" placeholder="Search...." @keypress="callWeather">
      </div>

      <div class="weather-wrap" v-if="typeof weather.main != 'undefined'">
        <div class="location-box" >
          <div class="location">{{ weather.name}}, {{weather.sys.country}}</div>
          <div class="date">{{ dateBuilder() }}</div>
        </div>

        <div class="weather-box">
          <div class="temp">{{ weather.main.temp }}℃</div>
          <div class="weather">{{ weather.weather[0].main }}</div>
        </div>
      </div>
      
    </main>
  </div>
</template>

<script>
export default {
  name: 'App',
  data() {
    return {
      area:'',
      api_key: '1f3b2cd75578ad56009aa3ac05fcc0c1',
      url_base: 'https://api.openweathermap.org/data/2.5/weather',
      weather: {}
    }
  },
  methods: {
    async callWeather(e){
      if( e.key == 'Enter' )
      {
        const weatherInfo = await fetch(`${this.url_base}?q=${this.area}&units=metric&appid=${this.api_key}`);
        const res = await weatherInfo.json();
        this.weather = res;
      }
    },
    dateBuilder () {
      let d = new Date();
      let months = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
      let days = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
      let day = days[d.getDay()];
      let date = d.getDate();
      let month = months[d.getMonth()];
      let year = d.getFullYear();
      return `${day} ${date} ${month} ${year}`;
    }
  },
}
</script>

<style>
* {
  margin: 0 auto;
  box-sizing: border-box;
}

body {
  font-family: 'montserrat', sans-serif;
}


#app {
  max-width: 360px;
  max-height: 640px;

  background-size: cover;
  background-position: bottom;
  transition: 0.4s;
  background-origin: 0;
}

#app .cold {
  background-image: url('./assets/cold-bg.jpeg');
}

#app .warm {
  background-image: url('./assets/warm-bg.jpeg');
}

main {
  text-align: center;
  min-height: 640px;
  padding: 25px;
  background-image: linear-gradient(to bottom, rgba(0, 0, 0, 0.25), rgba(0, 0, 0, 0.75));
}

.search-box {
  width: 100%;
  margin-bottom: 30px;
}

.search-box .search-bar {
  display: block;
  width: 100%;
  padding: 15px;

  color: #313131;
  font-size: 20px;

  appearance: none;
  border: none;
  outline: none;
  background: none;

  box-shadow: 0px 0px 8px rgba(0, 0, 0, 0.25);
  background-color: rgba(255, 255, 255, 0.5);
  border-radius: 0px 16px 0px 16px;
  transition: 0.4s;
}

.search-box .search-bar:focus{
  box-shadow: 0px 0px 16px rgba(0, 0, 0, 0.25);
  background-color: rgba(255, 255, 255, 0.5);
  border-radius: 16px 0px 16px 0px;
}

.location-box .location {
  color: #fff;
  font-size: 20px;
  font-weight: 500;
  text-shadow: 1px 3px rgba(0, 0, 0, 0.25);
}

.location-box .date {
  color: #fff;
  font-size: 20px;
  font-weight: 300;
  font-style: italic;
}
.weather-box .temp {
  display: inline-block;
  padding: 10px 10px;
  color: #fff;
  font-size: 80px;
  font-weight: 900;

  text-shadow: 3px 6px rgba(255, 255, 255, 0.25);
  background-color: rgba(255, 255, 255, 0.25);
  border-radius: 16px;
  margin: 10px;

  box-shadow: 3px 6px rgba(0, 0, 0, 0.25);
}

.weather-box .weather {
  color: #fff;
  font-size: 48px;
  font-weight: 700;
  font-style: italic;
  text-shadow: 3px 6px rgba(0, 0, 0, 0.25);
}
</style>
